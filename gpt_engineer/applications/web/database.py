...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_traces_session_id ON traces(session_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_trace_files_trace_id ON trace_files(trace_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cli_outputs_session_id ON cli_outputs(session_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cli_outputs_trace_id ON cli_outputs(trace_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_cli_outputs_timestamp ON cli_outputs(timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_engine_checkpoints_session_id ON engine_checkpoints(session_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_engine_checkpoints_unresolved ON engine_checkpoints(session_id, is_resolved)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_session_files_session_id ON session_files(session_id)")
...
class SessionRepository:
    ...
    def create_session(self, session_id: str, project_path: str, metadata: Dict = None) -> bool:
        ...
        with self.db.get_connection() as conn:
            try:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (session_id, project_path, current_time, current_time, metadata_json))
                return True
...
class TraceRepository:
    ...
    def create_trace(self, trace_id: str, session_id: str, prompt: str, mode: str = None) -> bool:
        """Create a new trace."""
        with self.db.get_connection() as conn:
            try:
                conn.execute("""
    ...
    def update_trace_execution_result(self, trace_id: str, execution_result: Dict) -> bool:
...
class CLIOutputRepository:
    ...
    def add_output(self, session_id: str, message: str, trace_id: str = None, 
    ...
    def clear_session_outputs(self, session_id: str) -> bool:
        ...
        with self.db.get_connection() as conn:
            ...
            self.add_output(session_id, "[STREAM_CLEARED]", message_type="stream_cleared")
            return True
...
class CheckpointRepository:
    ...
    def save_checkpoint(self, session_id: str, checkpoint_type: str, checkpoint_data: Dict) -> int:
    ...
    def get_pending_checkpoint(self, session_id: str, checkpoint_type: str = None) -> Optional[Dict]:
    ...
    def clear_session_checkpoints(self, session_id: str) -> bool:
        ...
        with self.db.get_connection() as conn:
            conn.execute("DELETE FROM engine_checkpoints WHERE session_id = ?", (session_id,))
            return True
